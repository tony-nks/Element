...
while($row = $db->get_row()) {
	...
	if (count(explode(".", $row['from'])) != 4 ) $from = "<a href=\"?mod=editusers&action=edituser&user=".urlencode($row['from'])."\" target=\"_blank\">{$row['from']}</a><br /><br /><a class=\"btn bg-brown-600 btn-sm btn-raised\" href=\"" . $config['http_home_url'] . "index.php?do=pm&doaction=newpm&username=".urlencode($row['from'])."\" target=\"_blank\">{$lang['send_pm']}</a>";
	else $from = $row['from'];
