						...
						<div class="panel-body">
							...
							<div class="form-group">
							 ...
							 <div class="form-group">
							  ...
							  <div class="col-sm-10">
								<input data-rel="calendar" type="text" name="newdate" class="form-control" style="width:190px;"><i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right position-left" data-rel="popover" data-trigger="hover" data-placement="right" data-content="{$lang[hint_calendar]}" ></i>{$author_info}
							  </div>
							</div>
...
HTML;
	...
	if( $user_group[$member_id['user_group']]['allow_main'] ) $main_input = "<div class=\"checkbox\"><label><input class=\"icheck\" type=\"checkbox\" id=\"allow_main\" name=\"allow_main\" value=\"1\" checked>{$lang['addnews_main']}</label></div>"; else $main_input = "";
...
{$output}
							<div class="form-group">
							  ...
							  <div class="col-md-10">
								  <div class="row">
									<div class="col-sm-6" style="max-width:300px;"><div class="checkbox"><label><input class="icheck" type="checkbox" id="approve" name="approve" value="1" checked>{$lang['addnews_mod']}</label></div></div>
									<div class="col-sm-6"><div class="checkbox"><label><input class="icheck" type="checkbox" id="allow_comm" name="allow_comm" value="1" checked>{$lang['addnews_comm']}</label></div></div>
								  </div>
								  <div class="row">
								  ...
								  <div class="row">
									<div class="col-sm-6" style="max-width:300px;"><div class="checkbox"><label><input class="icheck" type="checkbox" id="allow_rating" name="allow_rating" value="1" checked>{$lang['addnews_allow_rate']}</label></div></div>
...
elseif( $action == "doaddnews" ) {
	...
	$db->query( "INSERT INTO " . PREFIX . "_post_extras (news_id, allow_rate, votes, disable_index, access, user_id, disable_search, need_pass) VALUES('{$row}', '{$allow_rating}', '{$add_vote}', '{$disable_index}', '{$group_regel}', '{$userid}', '{$disable_search}', '{$need_pass}')" );
    ...
    if ( $need_pass ) {
		...
		$db->query( "INSERT INTO " . PREFIX . "_post_pass (news_id, password) VALUES('{$row}', '{$post_password}')" );		
	}
	...
	if( $expires AND $expires_action) {
		...
		$db->query( "INSERT INTO " . PREFIX . "_post_log (news_id, expires, action, move_cat) VALUES('{$row}', '$expires', '$expires_action', '$movecat_list')" );
	}
	...
	msg( "success", $lang['addnews_ok'], $lang['addnews_ok_1'] . " \"" . stripslashes( stripslashes( $title ) ) . "\" " . $lang['addnews_ok_2'], array('?mod=addnews&action=addnews' => $lang['add_s_1'], '?mod=editnews&action=editnews&id='.$row => $lang['add_s_2'], '?mod=editnews&action=list' => $lang['add_s_3'] ) );
}
?>